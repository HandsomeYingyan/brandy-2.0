...
int card_erase_boot0(uint length, void *buffer, uint storage_type)
{
	...
	if (storage_type == STORAGE_EMMC) {
		...
		ret = sunxi_flash_phywrite(BOOT0_SDMMC_BACKUP_START_ADDR,
					   length / 512, buffer);
		if (!ret) {
		...
		ret = sunxi_flash_phywrite(BOOT0_SDMMC_START_ADDR, length / 512,
					   buffer);
		if (!ret) {
			debug("%s: write boot0 from %d fail\n", __func__,
		...
		ret = sunxi_flash_phywrite(BOOT0_EMMC3_START_ADDR, length / 512,
					   erase_buffer);
		if (!ret) {
			debug("%s: write boot0 from %d fail\n", __func__,
		...
		ret = sunxi_flash_phywrite(BOOT0_EMMC3_BACKUP_START_ADDR,
...
int card_erase(int erase, void *mbr_buffer)
{
	...
	for (i = 1; i < mbr->PartCount; i++) {
		...
		if (ret == 0) {
			...
			if (!sunxi_flash_write(erase_head_addr,
					       erase_head_sectors,
					       erase_buffer)) {
				debug("card erase fail in erasing part %s\n",
				       mbr->array[i].name);
			...
			if (erase_tail_sectors) {
				if (!sunxi_flash_write(erase_tail_addr,
						       erase_tail_sectors,
						       erase_buffer)) {
					debug("card erase fail in erasing part %s\n",
					       mbr->array[i].name);
