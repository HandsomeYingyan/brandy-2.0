...
static int parse_replace_fdt(char *replace_fdt,
	...
	printf("replace_fdt is longer than 256 characters\n");
...
static int __sunxi_replace_fdt(void * buffer)
{
	...
	memcpy(gd->fdt_blob, buffer, new_fdt_totalsize);
	...
	if (!sunxi_partition_get_info(partition_name, &disk_part_info)) {
		...
		if (buffer) {
			...
			if (ret) {
				...
				printf("replaced fdt : read raw partition(%s): start,0x%x sectors 0x%x fail\n",
				 partition_name, disk_part_info.start, disk_part_info.size);
				goto error;
			}
			printf("replace fdt from raw partition(%s) success\n", partition_name);
		} else {
			printf("replace fdt : malloc fail\n");
			return -1;
		}
	} else {
...
static int sunxi_replace_fdt_from_fat(char *partition_name, char *file_name)
{
	...
	if (partno >= 0){
		...
		if ( filesize <= 0 ) {
			printf("Please check if the file(%s) exists\n",file_name);
			return -1;
		}

		if (filesize > gd->fdt_size) {
			printf("filesize(%u) biger than gd->fdt_size(%u)\n",filesize , gd->fdt_size);
			return -1;
		}
		...
		if (buffer) {
			...
			if (!__load_fdt_file_from_fat(partno, file_name, buffer)) {
				...
				printf("replaced fdt : read fat partition(%s) file(%s) fail\n",
				 partition_name, file_name);
				goto error;
			}
			printf("replace fdt from fat partition(%s) file(%s) success\n",
					 partition_name, file_name);
		} else {
			printf("replaced fdt : malloc fail\n");
			return -1;
		}
	} else {
		printf("%s partition is not exist\n", partition_name);
...
int sunxi_replace_fdt(void)
{
	...
	if (replace_fdt) {
		...
		if (strcmp(partition_type, "raw") == 0) {
			printf("replaced fdt : partition type (%s)\n", partition_type);
			if (sunxi_replace_fdt_from_raw(partition_name)) {
				printf("replace fdt form raw partition(%s) fail!\n", partition_name);
			}
		} else if (strcmp(partition_type, "fat") == 0) {
			printf("replaced fdt : partition type (%s)\n", partition_type);
			if (sunxi_replace_fdt_from_fat(partition_name,file_name)) {
				printf("replace fdt form fat partition(%s) file(%s) fail!\n",
					 partition_name, file_name);
			}
		} else {
			printf("replaced fdt error : partition type (%s) not support\n", partition_type);
		}
	} else {
		return 0;
	}
