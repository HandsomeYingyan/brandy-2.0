	...
	if (work_mode == WORK_MODE_BOOT) {
		...
		if (part_start != 0) {
			*dtb_data = CONFIG_SUNXI_FDT_ADDR;
			sunxi_flash_read(part_start, ALIGN(sizeof(struct fdt_header), 512)/512, (char *)(*dtb_data));
		...
		if (fdt_check_header((void *)*dtb_data) < 0) {
			...
			sunxi_flash_read(part_start, ALIGN(sizeof(struct andr_img_hdr), 512)/512, (char *)boot_head_addr);
			android_image_get_dtb((const struct andr_img_hdr *)boot_head_addr, dtb_data, dtb_len);
		}
	}
	return 0;
}
...
int do_set_working_fdt(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	if (argc < 2) {
		...
		printf("change working_fdt 0x%x to ", (uint)working_fdt);
