...
#endif
{
	...
	do {
		...
		if (mmc_read_blocks(mmc, dst, start, cur) != cur) {
			MMCMSG(mmc, "%s: Failed to read blocks\n", __func__);
			return 0;
		}
		blocks_todo -= cur;
		start += cur;
...
static int mmc_select_mode_and_width(struct mmc *mmc, uint card_caps)
{
	...
	if (err)
		pr_err("sunxi switch to best bus failed\n");
	return err;
...
static int mmc_startup(struct mmc *mmc)
{
	...
	pr_err("the mode %s (freq : %d MHz)\n", mmc_mode_name(mmc->best_mode),
		 mmc->tran_speed / 1000000);
...
int mmc_init(struct mmc *mmc)
{
	...
	if ((cfg->sample_mode == AUTO_SAMPLE_MODE)
		...
		/*
		pr_info("-----------sunxi switch to best bus\n");
		err = sunxi_switch_to_best_bus(mmc);
		if (err) {
			pr_info("switch to best speed mode fail\n");
			goto ERR_RET;
