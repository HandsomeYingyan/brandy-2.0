...
int sunxi_mmc_tuning_init(void)
{
	...
	if (tuning_blk_8b == NULL) {
		printf("%s: request memory for buf_bus8 fail\n", __FUNCTION__);
		return -1;
	}

	if (tuning_blk_4b == NULL) {
		printf("%s: request memory for buf_bus4 fail\n", __FUNCTION__);
		ret = -1;
		goto OUT;
	}
	...
	for (i = 0; i < MAX_EXT_FREQ_POINT_NUM; i++) {
		...
		if (val & (1U<<31)) {
			if ((((val>>8) & 0xff) == freq_index) && (((val>>16) & 0xff) == speed_md)) {
				...
				printf("select ext freq point: %d-%d MHz\n", i, (val & 0xff));
				goto OUT;
			}
		}
	}
...
static int sunxi_tuning_method_0(struct mmc *mmc, int retry_times)
{
	...
	if (rcv_pat == NULL) {
		printf("request memory for rcv_pat fail\n");
		return -1;
	}
	...
	if (mmc->bus_width == 4) {
		...
		printf("Not support 1 bit tuning now\n");
		ret = -1;
		goto OUT;
	}
...
static int sunxi_tuning_speed_mode(struct mmc *mmc, int speed_mode, int tuning_mode, int retry_times)
{
	...
	if ((freq != 0) && (sdly_cfg[speed_mode*MAX_CLK_FREQ_NUM + freq_index] != 0xFF)) {
		...
		if ((freq != 0) && (sdly_cfg[speed_mode*MAX_CLK_FREQ_NUM + freq_index] != 0xFF))
			...
			printf("invalid freq, switch to %d\n", freq_def);
		}
	}
	/* update sample point cfg */
	mmc_set_clock(mmc, mmc->tran_speed, false);
...
static int sunxi_tuning_hs400_cmd(struct mmc *mmc, int speed_mode, int tuning_mode, int retry_times)
{
	...
	p = memalign(CONFIG_SYS_CACHELINE_SIZE, sdly_cnt*MAX_CLK_FREQ_NUM);
	if (speed_mode != HS400) {
		printf("%s: input speed mode error %d\n", __FUNCTION__, speed_mode);
		free(p);
		return 0;
	}
	...
	if ((freq != 0) && (sdly_cfg[speed_mode*MAX_CLK_FREQ_NUM + freq_index] != 0xFF)) {
		...
		ret = -ERR_NO_BEST_DLY;
		mmc->tran_speed = freq_def;
		printf("invalid freq, switch to 6MHz\n");
	}
...
int write_tuning_try_freq(struct mmc *mmc, u32 clk)
{
	...
	if (rcv_pattern == NULL) {
		printf("%s: request memory for rcv_pattern fail\n", __FUNCTION__);
		return -1;
	}

	std_pattern = (char *)memalign(CONFIG_SYS_CACHELINE_SIZE, TUNING_LEN * 512);
	if (std_pattern == NULL) {
		printf("%s: request memory for std_pattern fail\n", __FUNCTION__);
		ret = -1;
		goto out1;
	}
	...
	if (mmc->bus_width == 4) {
		...
		printf("Don't support 1 bit tuning now\n");
		ret = -1;
		goto out;
	}
	...
	do {
		...
		if (ret != pat_blk_cnt) { //fail
			...
			printf("write_tuning_try_freq: write ok\n");
			...
			if (ret != pat_blk_cnt) {
				...
				if (ret) {
					...
					printf("Pattern compare ok\n");
					printf("Write tuning pattern ok\n");
					goto out;
				}
			}
		}
	} while (!mmc->cfg->ops->decide_retry(mmc, err_no, 0));
	printf(" Write tuning pattern failded\n");
	ret = -1;
...
int sunxi_execute_tuning(struct mmc *mmc, int speed_mode)
{
	...
	if (!IS_SD(mmc)) {
		...
		if (ret) {
			printf("switch to HS400 mode fail\n");
			goto OUT;
		}
	} else {
		if (speed_mode >= HSDDR52_DDR50) {
			printf("don't spport %s for sd card\n", spd_name[speed_mode]);
			ret = -1;
			goto OUT;
		}
	}
	...
	if (speed_mode == HS400) {
		...
		if (ret < 0) {
			printf("tuning hs400 cmd line fail\n");
			goto OUT;
		}
	}
...
int sunxi_bus_tuning(struct mmc *mmc)
{
...
#if 0
	printf("================== start tuning DS26_SDR12...\n");
	ret = sunxi_execute_tuning(mmc, DS26_SDR12);
	if (ret) {
		printf("tuning fail at DS26_SDR12\n");
		err = -1;
	}
#endif

	printf("================== HSSDR52_SDR25...\n");
	ret = sunxi_execute_tuning(mmc, HSSDR52_SDR25);
	if (ret) {
		printf("tuning fail at HSSDR52_SDR25\n");
		err = -1;
		goto ERR_RET;
	}
	...
	if (mmc->host_caps & MMC_MODE_HS200) {
		...
		if (ret) {
			printf("tuning fail at HS200_SDR104\n");
			//err = -2;
			//goto ERR_RET;
			err_flag |= 0x1;
		}
	...
	if (mmc->host_caps & MMC_MODE_DDR_52MHz) {
		printf("================== HSDDR52_DDR50...\n");
		ret = sunxi_execute_tuning(mmc, HSDDR52_DDR50);
		if (ret) {
			printf("tuning fail at HSDDR52_DDR50\n");
			//err = -3;
			//goto ERR_RET;
			err_flag |= 0x2;
		}
	...
	if (((mmc->host_caps & (MMC_MODE_HS400|MMC_MODE_8BIT)) == (MMC_MODE_HS400|MMC_MODE_8BIT))
		...
		if (ret) {
			printf("switch back to HSDDR52_DDR50 8bit fail\n");
			err = -5;
			goto ERR_RET;
		}
	}
	...
	if (ret) {
		printf("switch back to HSSDR52_SDR25 8bit fail\n");
		err = -6;
		goto ERR_RET;
	}
...
int sunxi_switch_to_best_bus(struct mmc *mmc)
{
	...
	*/
	...
	imd = HSSDR52_SDR25;
	ifreq = CLK_50M;
	printf("use default speed mode: %d-%s, ifreq: %d\n", imd, spd_name[imd], ifreq);


	//printf("don't find best speed mode and freq\n");
	//ret = -1;
	//goto OUT;
...
START_SWITCH:
	...
	if (freq != 0)
		...
		printf("%s: err freq %d-%d\n", __func__, ifreq, freq);
		ret = -1;
		goto OUT;
	}

	printf("Best spd md: %d-%s, freq: %d-%d\n", imd, spd_name[imd], ifreq, freq);
...
int mmc_request_update_boot0(int dev_num)
{
	...
	if ((mmc->cfg->sample_mode == AUTO_SAMPLE_MODE)
		&& (mmc->tuning_end)) {
		printf("mmc request udpate boot0\n");
		return 1;
	} else
		return 0;
}
	...
	//if (work_mode != WORK_MODE_BOOT)
	{
		...
		if (mmc->cfg->sample_mode == AUTO_SAMPLE_MODE) {
			if ((sizeof(struct tune_sdly)+64) > buffer_size) { /* 64byte is resvered for other information */
				printf("size of tuning_sdly over %d\n", buffer_size);
			} else
